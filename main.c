...
int main()
{
	...
	med_stuff[0] = realloc(med_stuff[0], 100);
	large_stuff[0] = realloc(large_stuff[0], 1050);
	show_alloc_mem_ex();
	((char *)small_stuff[0])[5] = 0;
	((char *)med_stuff[0])[99] = 0;
	((char *)large_stuff[0])[1049] = 0;
	ft_putendl("This should display 'abcde':");
	ft_putendl(small_stuff[0]);
	ft_putendl("This should end with 'rhoncus preti':");
	ft_putendl(med_stuff[0]);
	ft_putendl("This should end with '. Nullam vel nu'");
