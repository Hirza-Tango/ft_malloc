...
/*   Updated: 2019/06/28 16:52:30 by dslogrov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_alloc_search	find_entry(void *ptr)
{
	...
	while (++i < ALLOC_NUM_SMALL)
		if (g_alloc_small[i].start == ptr)
			return (search_result(&(g_alloc_small[i]), i, ALLOC_SIZE_TINY, (i ==
				ALLOC_NUM_SMALL ? g_region_small.start + g_region_small.size
